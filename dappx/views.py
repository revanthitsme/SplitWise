...
def amount_of_each(request, friendslist, not_exists, user_given_in_list, empty):
    amount_list = []
    owed_list = []
    owed_friend_list = []
    owe_list = []
    ...
    return render(request, 'dappx/Friends.html',
                                    ...
                                    'user_given_in_list':user_given_in_list,'empty':empty,
...
def Friendstab(request):
    friendobj = Friends.objects.get_or_create(current_user=request.user)
    friends_of_curr_user = friendobj[0].Friendslist.all()
    # amount_list = amount_of_each(request, friends_of_curr_user, False, False, False)
    return amount_of_each(request, friends_of_curr_user, False, False, False)
...
def Transactions(request):
    if request.method == 'POST':
        ...
        if request.user.username in Receivers:
            return render(request, 'dappx/Transactions.html',{'user_given_in_list':True})
...
################################33
@login_required
def addfriends(request): ## amount_of_each(request, friendslist, not_exists, user_given_in_list, empty):
    if request.method == 'POST':
        ...
        if Friends_str is '':
            return amount_of_each(request, friends_of_curr_user, False, False, True)
        ...
        try:
            ...
            return amount_of_each(request, friends_of_curr_user, True, False, False)

        if request.user.pk in idlist:
            return amount_of_each(request, friends_of_curr_user, False, True, False)
...
def view_friend(request, pk):
    ...
    return render(request, 'dappx/view_friend.html',{'transactions':result1, 'friend':Friend2})
...
def tagplots(request):
    if request.method == 'POST':
                ...
                for k in dates_approved_objects:
                    if(k.Date2==j and k.Tag==i):
                        if(k.Donor.username==user.username):
                            money = money + k.shares[0]
                        else:
                            x=k.Receivers.index(user.username)
                            money = money + k.shares[x+1]
            ...
            for obj in objectfortags:
                ...
                if user.username in group_members:
                    if(obj.Donor.username==user.username):
                            money_spent = money_spent + k.shares[0]
                        else:
                            x=k.Receivers.index(user.username)
                            money_spent = money_spent + k.shares[x+1]
        ...
        for i in friends_names:
            ...
            money_value_for_friend.append(float(money))
            money_given_by_frnd.append(float(money2))
            money_given_by_me.append(float(money1))
        for i in friends_names:
            print(i)
        ...
        for i in group_names:
            ...
            for j in friends_objects2:
                if(j.Group==i):
                    money2=money2+j.Damount
            money_given_to_group.append(float(money1))
            money_given_to_me_by_group.append(float(money2))
        ...
        chart6 = {
            ...
            'series': [{
                ...
                'data':list(map(lambda row1,row2:{'name':row1,'y': row2},Tags_approved,Tags_numbers))
            }]
        }
        ##########
        ########### 
        ...
        dump6 = json.dumps(chart6)
        ##############


        return render(request,'dappx/plot.html',{'chart1':dump1,'chart2':dump2,'chart3':dump3,'chart4':dump4,'chart5':dump5,'excel':html_str,'chart6':dump6})
    else:
        return render(request,'dappx/time_series_plots.html')

def friend_transaction(request):
    if request.method == 'POST':
        Receiver = request.POST.get('Receiver')
        Amount = request.POST.get('Amount')
        Description = request.POST.get('Description')
        Tag = request.POST.get('Tag')
        user = User.objects.get(id=request.user.id)
        if Receiver is '' or Description is '' or Amount is '':
            return render(request, 'dappx/friend_transaction.html',{'receiver_none':True})
        if Receiver is request.user.username:
            return render(request, 'dappx/friend_transaction.html',{'user_given_in_list':True})
        friendobj = Friends.objects.get_or_create(current_user=request.user)
        friends_of_curr_user = friendobj[0].Friendslist.all()
        friendnamelist = [obj.username for obj in friends_of_curr_user]
        if Receiver not in friendnamelist:
            return render(request, 'dappx/friend_transaction.html',{'not_friend':True})
        print(friendobj)
        print(friends_of_curr_user)
        try:
            Friend = User.objects.get(username=Receiver)
        except Exception as e:
            return render(request, 'dappx/friend_transaction.html',{'NoReceivers':True})
        Tags_approved = ["Movies","Food","Housing","Travel","Others"]

        if Tag not in Tags_approved:
            return render(request,'dappx/friend_transaction.html',{'Tag_not_approved':True})
        if float(Amount)>=0:
        ...
        Activity_obj.save()
        return render(request,'dappx/friend_transaction.html')
    else:
        return render(request,'dappx/friend_transaction.html')